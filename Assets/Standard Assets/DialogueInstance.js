...
function Update () 
{
    ...
    {
		case 2:
		if (Input.GetKeyDown("return") || firePressed) EvaluatePassword();
		break;
	}
...
function DoNextButton () 
{
	...
	case 0:
        ...
        {
		    if (firePressed || GUI.Button(Rect(Screen.width-84,Screen.height-84,64,64),"Next","arrow"))
            {
                ProgressLineCount();			
		        firePressed = false;
            }
...
function ShowWheel () 
{
    ...
    if (activeItem >= display.choices.length)
        activeItem = 0;
    else if (activeItem < 0)
        activeItem = display.choices.length - 1;
    if (!mouseMoved)
    {
        GUI.FocusControl( display.choices[activeItem].shortText);
    }
    else
    {
        GUI.FocusControl("");
    ...
    {
        GUI.SetNextControlName(display.choices[0].shortText);
        GUI.Label(
            Rect(1,Screen.height-163+1,Screen.width/2,43),
            display.choices[0].shortText,
            "r1c1black");

		if ((firePressed && activeItem == 0) || GUI.Button(
            Rect(0,Screen.height-163,Screen.width/2,43),
            display.choices[0].shortText,
            "r1c1active"))
        {
            firePressed = false;
    ...
    {
        GUI.SetNextControlName(display.choices[1].shortText);
        (GUI.Label(Rect(Screen.width/2+1,Screen.height-163+1,Screen.width/2,43),display.choices[1].shortText,"r1c2black")) ;

		if ((firePressed && activeItem == 1) || GUI.Button(Rect(Screen.width/2,Screen.height-163,Screen.width/2,43),display.choices[1].shortText,"r1c2active"))
        {
    ...
    {
        GUI.SetNextControlName(display.choices[2].shortText);
            (GUI.Label(Rect(0+1,Screen.height-120+1,Screen.width/2,40),display.choices[2].shortText,"r2c1black"));

		if ((firePressed && activeItem == 2) || GUI.Button(Rect(0,Screen.height-120,Screen.width/2,40),display.choices[2].shortText,"r2c1active"))
        {
    ...
    {
        GUI.SetNextControlName(display.choices[3].shortText);
        GUI.Label(Rect(Screen.width/2+1,Screen.height-120+1,Screen.width/2,40),display.choices[3].shortText,"r2c2black");

		if ((firePressed && activeItem == 3) || GUI.Button(Rect(Screen.width/2,Screen.height-120,Screen.width/2,40),display.choices[3].shortText,"r2c2active"))
        {
    ...
    {
        GUI.SetNextControlName(display.choices[4].shortText);
        GUI.Label(Rect(0+1,Screen.height-80+1,Screen.width/2,51),display.choices[4].shortText,"r3c1black");

		if ((firePressed && activeItem == 4) || GUI.Button(Rect(0,Screen.height-80,Screen.width/2,51),display.choices[4].shortText,"r3c1active")) 
        {
    ...
    {
        GUI.SetNextControlName(display.choices[5].shortText);
        GUI.Label(Rect(Screen.width/2+1,Screen.height-80+1,Screen.width/2,51),display.choices[5].shortText,"r3c2black");
		if ((firePressed && activeItem == 5) || GUI.Button(Rect(Screen.width/2,Screen.height-80,Screen.width/2,51),display.choices[5].shortText,"r3c2active"))
        {
...
private var activeItemChanged = false;
...
function FixedUpdate () 
{
    ...
    firePressed = Input.GetButtonUp("Fire1");
    ...
    var deadZone = 0.2;
    if ((Input.GetAxis("Vertical") > deadZone || Input.GetAxis("Horizontal") > deadZone) && !activeItemChanged)
    {
        mouseMoved = false;
        activeItemChanged = true;
        activeItem++;
    }
    else if ((Input.GetAxis("Vertical") < -deadZone || Input.GetAxis("Horizontal") < -deadZone) && !activeItemChanged)
    {
        mouseMoved = false;
        activeItemChanged = true;
        activeItem--;
    }
    else if (Mathf.Abs(Input.GetAxis("Vertical")) < deadZone && Mathf.Abs(Input.GetAxis("Horizontal")) < deadZone)
    {
